...
class CEvaluation {
    ...
    private:
        ...
        void set_situation_for_one_direction(char x,char y ,short countx,short county,int dir,char board[][GRID_NUM]);
    ...
    private:
        ...
        int         m_w_cross;
        int         m_b_cross;
        int         m_w_detail[15][15];  //用以记录白棋交叉的详细情况
        int         m_b_detail[15][15];   //黑棋
        char        m_visited_direction[20][20][4];

        int         m_w_vir_detail[15][15];  //用以记录白棋交叉的详细情况
        int         m_b_vir_detail[15][15];   //黑棋
